...
- Name: as1360-complete.spec.js >> 🟢 BULK TESTING — Upload, Results & Responsiveness >> ✅ TC-B01 | Bulk Testing tab loads correctly
- Location: as1360-complete.spec.js:186:3

# Error details

```
Error: Upload area visible

expect(locator).toBeVisible() failed

Locator: locator('text=Click to upload').first()
Expected: visible
Timeout: 5000ms
Error: element(s) not found

Call log:
  - Upload area visible with timeout 5000ms
  - waiting for locator('text=Click to upload').first()

```

```yaml
- img
- heading "Virtual Coordinator Setup" [level=1]
- img "close"
- img
- text: Create Coordinator Add details to your project
- img
- text: Train Coordinator Upload data to start the training process
- img
- text: Create Content Manage VC response content
- img
- text: Configure Notifications Manage alerts based on VC categorizations
- img
- text: Test Coordinator Test the VC with example HCP messages
- heading "Configure Notifications (Optional)" [level=2]
- paragraph: Manage alerts based on VC categorizations
- text: When category is selected
- combobox
- text: e.g. General Info, Treatment Recommendation, Safety Notify
- combobox
- text: e.g. People (drop down list of accounts)
- heading "Channels Notification delivery channels" [level=2]
